...
import cn.itcast.erp.utils.file.PoiUtil;
...
public class ContractPrint{
	...
	public void print(Contract contract,String path, HttpServletResponse response) throws Exception{
		...
		//将oSet中的数据复制到oList中
		//直接将oSet中的数据放入oList中，有问题吗？
		//答：有，因为oSet中的数据进入oList的时候，还是无序的，还是需要反反复复的试探性的查找
		oList1.addAll(oSet);
		
		//需要根据工厂的名字 对oList中的数据进行分组归类，同名的工厂对应的货物应该靠在一起存放
		//先获取所有的工厂，放在一个工厂名字的集合中
		List<String> nameList = new ArrayList<String>();
		
		for(ContractProduct cp:oList1){
			if(!nameList.contains(cp.getFactoryName())){
				nameList.add(cp.getFactoryName());
			}
		}
		
		//遍历工厂名字，然后去oList中查找对应的货物信息，放入到新的集合中
		List<ContractProduct> oList = new ArrayList<ContractProduct>();
		for(String name:nameList){
			for(ContractProduct cp:oList1){
				if(cp.getFactoryName().equals(name)){
					oList.add(cp);
				}
			}
		}
		...
		//Iterator<ContractProduct> it = oList.iterator();
...
//		sheet.removeRowBreak(6);
		...
		for(int p=0;p<pageList.size();p++){
			...
			//if(contract.getPrintStyle().equals("2") && UtilFuns.isNotEmpty(printMap.get("ProductNo2"))){
				...
				//TotalAmount
				//nRow = sheet.createRow(curRow-1);//11-1=10
		//		nRow.setHeightInPoints(24);
				if(UtilFuns.isNotEmpty(printMap.get("Cnumber"))&&UtilFuns.isNotEmpty(printMap.get("Price"))){
					nCell  = nRow.createCell(8);
					nCell.setCellType(HSSFCell.CELL_TYPE_FORMULA);
