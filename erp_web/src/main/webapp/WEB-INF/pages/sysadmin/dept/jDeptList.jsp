<%@page import="cn.itcast.erp.common.SysConstant"%>
...
<%-- ${_CURRENT_USER.userInfo.id}
<c:forEach var="a" items="${_CURRENT_USER.roles }">
${a.modules }
...
<table id="ec_table" class="tableRegion" width="98%" >
	...
	<c:forEach items="${results }" var="dept"  varStatus="st">
		<tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'" >
			...
			<td align="left">${st.count }</td>
			<td align="left">${dept.id }</td>
			<td align="left">${dept.parent.deptName }</td>
			<td align="left"><a href="deptAction_toview?id=${dept.id }">${dept.deptName }</a></td>
			<td align="left">${dept.state==0?"<font color='red'>禁用</font>":"<font color='green'>启用</font>" }</td>
		</tr>
   </c:forEach>
	</tbody>
</table>
